...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0] < options.defaultDate) (0, _notiflix.Notify).failure(`Please choose a date in the future`);
        else {
...
function onClickBtn() {
    ...
    function timer() {
        ...
        const days = addLeadingZero(Math.floor(diff / 86400000));
        const hours = addLeadingZero(Math.floor(diff / 3600000 % 24));
        const minutes = addLeadingZero(Math.floor(diff / 60000 % 60));
        ...
        if (diff < 1000) {
            ...
            startBtn.removeAttribute(`disabled`);
            let str = "\u2665";
            (0, _notiflixDefault.default).Notify.success("Time to rest a little!" + str);
        }
    }
